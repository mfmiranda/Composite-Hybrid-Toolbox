...
%%%% Get_DWT(wavelet,n,nlevels,boundary): Compute W matrix
%%%%
...
%%%%                    'reflection': reflect
%%%%                    'per       ': periodic, Matlab "per" (repeat last
%%%%                                        if odd
%%%%                    'zero      ': pad with zeros
%%%%                    '**********': anything else assume ppd in Matlab
...
Kj=repmat(0,J+1,1);
for (j=1:J);
    H1A=get_H1A(nn,h,boundary,extend); %%% Scaling filter
    G1A=get_H1A(nn,reverse(h),boundary,extend);  %%% Wavelet Filter
    H1B=get_H1B(nn,h,boundary,extend);
    G1B=get_H1B(nn,reverse(h),boundary,extend);
    H{j}=H1A+H1B; 
    G{j}=G1A+G1B;
    nn=size(H{j},1);
    %if (boundary=='per       ')&(nn/2~=floor(nn/2))
    %    nn=nn+1;
    %end;
    Kj(J+2-j)=size(G{j},1);
