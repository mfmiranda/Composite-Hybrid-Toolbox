...
for (i=1:(N-1))
    if strcmp(boundary,'reflection')
        H1B(ictr,(1:(2*(N-i))))=h((2*(N-i)):-1:1);
    elseif (strcmp(boundary,'per')&(n/2~=floor(n/2)))
...
%%%%
eps=1e-6;
keep_extra_flag=(sum(h)>eps)|min(strcmp(boundary,'per'));
if extend==0
