...
path3=strcat(path2,'Datasets',sep);
...
for dataset=1:100
filename=strcat(path3,'DataShortTermNoise_',num2str(dataset));
load(filename)
X0=Y_MatrixC;
...
PC_X0=X0*V0(:,1:p0);
Y_star=PC_X0';
 %PC_X0 is the matrix of global features
...
qqV0Criteria=quantile(V0Criteria,[0.3,0.6,0.9]);
wavespecsPC.Kj(1)=sum(V0Criteria>=qqV0Criteria(3));
wavespecsPC.Kj(2)=sum(V0Criteria>=qqV0Criteria(2) & V0Criteria<qqV0Criteria(3));
wavespecsPC.Kj(3)=sum(V0Criteria>=qqV0Criteria(1) & V0Criteria<qqV0Criteria(2));
...
save(strcat(path2,'ResultsSim100_GSB_ShortTerm'),'MSEArray','FPArray', 'RPArray', 'IntervalWidthArray')
